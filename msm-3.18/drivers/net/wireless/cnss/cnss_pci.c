...
#define WLAN_QCA_VREG_EN_1P8    "wlan-qca-vreg-en-1p8"
#define WLAN_QCA_VREG_EN_3P3    "wlan-qca-vreg-en-3p3"
...
static struct cnss_data {
	...
	int wlan_qca_vreg_en_1p8;
	int wlan_qca_vreg_en_3p3;
...
static int cnss_wlan_pmi_vreg_1p8_init(void){

	int ret = 0;

	ret = gpio_request(penv->wlan_qca_vreg_en_1p8,WLAN_QCA_VREG_EN_1P8);
	if (ret) {
		pr_err("%s: Can't get GPIO %s, ret = %d\n",
				__func__, WLAN_QCA_VREG_EN_1P8, ret);
		goto out;
	}

	ret = gpio_direction_output(penv->wlan_qca_vreg_en_1p8,
			WLAN_BOOTSTRAP_HIGH);
	if (ret) {
		pr_err("%s: Can't set GPIO %s direction, ret = %d\n",
				__func__, WLAN_QCA_VREG_EN_1P8, ret);
		gpio_free(penv->wlan_qca_vreg_en_1p8);
		goto out;
	}
...
static int cnss_wlan_pmi_vreg_3p3_init(void){

	int ret = 0;

	ret = gpio_request(penv->wlan_qca_vreg_en_3p3,WLAN_QCA_VREG_EN_3P3);
	if (ret) {
		pr_err("%s: Can't get GPIO %s, ret = %d\n",
				__func__, WLAN_QCA_VREG_EN_3P3, ret);
		goto out;
	}

	ret = gpio_direction_output(penv->wlan_qca_vreg_en_3p3,
			WLAN_BOOTSTRAP_HIGH);
	if (ret) {
		pr_err("%s: Can't set GPIO %s direction, ret = %d\n",
				__func__, WLAN_QCA_VREG_EN_3P3, ret);
		gpio_free(penv->wlan_qca_vreg_en_3p3);
		goto out;
	}
...
static int cnss_get_wlan_power_gpio(struct platform_device *pdev)
{
	int ret = 0;
	/*Enabling QCA_VREG_EN_1P8 & 3P3*/
	if (of_find_property((&pdev->dev)->of_node,
				WLAN_QCA_VREG_EN_1P8, NULL)) {
		penv->wlan_qca_vreg_en_1p8 =
			of_get_named_gpio((&pdev->dev)->of_node,
					WLAN_QCA_VREG_EN_1P8, 0);
		if (penv->wlan_qca_vreg_en_1p8 > 0) {
			ret = cnss_wlan_pmi_vreg_1p8_init();
		} else {
			ret = penv->wlan_qca_vreg_en_1p8;
			pr_err(
					"%s: Can't get GPIO %s, ret = %d",
					__func__, WLAN_QCA_VREG_EN_1P8, ret);
		}
	...
	if (of_find_property((&pdev->dev)->of_node,
				WLAN_QCA_VREG_EN_3P3, NULL)) {
		penv->wlan_qca_vreg_en_3p3 =
			of_get_named_gpio((&pdev->dev)->of_node,
					WLAN_QCA_VREG_EN_3P3, 0);
		if (penv->wlan_qca_vreg_en_3p3 > 0) {
			ret = cnss_wlan_pmi_vreg_3p3_init();
		} else {
			ret = penv->wlan_qca_vreg_en_3p3;
			pr_err(
					"%s: Can't get GPIO %s, ret = %d",
					__func__, WLAN_QCA_VREG_EN_3P3, ret);	
		}
...
static int cnss_wlan_get_resources(struct platform_device *pdev)
{
	...
	ret = cnss_get_wlan_power_gpio(pdev);
	if (ret) {
		pr_err("%s: Failed to enable wlan vreg_1p8 and 3p3 gpio\n", __func__);
		goto err_gpio_wlan_power_pin;
	}


	return ret;

err_gpio_wlan_power_pin:
	//cnss_put_wlan_power_gpio();
...
again:
	...
	if (penv->wlan_qca_vreg_en_1p8 > 0) {
		gpio_set_value(penv->wlan_qca_vreg_en_1p8, WLAN_BOOTSTRAP_HIGH);
		msleep(WLAN_BOOTSTRAP_DELAY);
	}
	if (penv->wlan_qca_vreg_en_3p3 > 0) {
		gpio_set_value(penv->wlan_qca_vreg_en_3p3, WLAN_BOOTSTRAP_HIGH);
		msleep(WLAN_BOOTSTRAP_DELAY);
	}
...
static int cnss_remove(struct platform_device *pdev)
{
	...
	if (penv->wlan_qca_vreg_en_1p8 > 0)
		gpio_set_value(penv->wlan_qca_vreg_en_1p8, WLAN_BOOTSTRAP_LOW);
	if (penv->wlan_qca_vreg_en_3p3 > 0)
		gpio_set_value(penv->wlan_qca_vreg_en_3p3, WLAN_BOOTSTRAP_LOW);
	cnss_wlan_release_resources();
