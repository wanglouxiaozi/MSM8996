		...
		struct platform_device *pdev)
{
	...
	if (socinfo_get_platform_subtype()) {
		/* Only OSD display supported for uSOM 820*/
		dsi_pan_node = of_parse_phandle(pdev->dev.of_node,
				"qcom,dsi-pref-prim-pan", 0);
	} else {
		/*GPIO81 will be High for OSD display */
		if (gpio_get_value(ctrl_pdata->panel_ident_gpio))
			dsi_pan_node = of_parse_phandle(pdev->dev.of_node,
					"qcom,dsi-pref-prim-pan1", 0);
		else
			dsi_pan_node = of_parse_phandle(pdev->dev.of_node,
					"qcom,dsi-pref-prim-pan0", 0);
	}
	if (!dsi_pan_node)
...
/*
   Display identification pin (gpio81) is only connected on Open-Q 820.
   GPIO81 will be  High in case of OSD display and LOW in case of
   Truly(with synaptic touch) display
   */
int mdss_dsi_parse_panel_identification(struct platform_device *ctrl_pdev,
		struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	int ret = 0;

	ret = socinfo_get_platform_subtype();
	if (ret) {
		return ret;
	} else {
		ctrl_pdata->panel_ident_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
				"qcom,panel-ident-gpio", 0);
		if (!gpio_is_valid(ctrl_pdata->panel_ident_gpio))
			pr_err("%s:%d, Panel Identification  gpio not specified\n",
					__func__, __LINE__);
		ret = gpio_get_value(ctrl_pdata->panel_ident_gpio);
	}
	return ret;
}
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	if (mdss_dsi_parse_panel_identification(pdev,ctrl_pdata)) {
		pr_info("%s: OSD Display Panel detected\n", __func__);
	} else {
		pr_info("%s: TRULY Display Panel detected\n", __func__);
	}
