...
static int msm_ssphy_qmp_reg_addr_show(struct seq_file *s, void *unused)
{
...
static int msm_ssphy_qmp_reg_addr_open(struct inode *inode, struct file *file)
{
	return single_open(file, msm_ssphy_qmp_reg_addr_show, inode->i_private);
}

static ssize_t msm_ssphy_qmp_reg_addr_write(struct file *file,
		const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	ret = kstrtou32_from_user(ubuf, count, 0, &reg_addr);
	if (ret)
		return ret;

	if ((reg_addr < 0x07410000) || (reg_addr > 0x07410794))
		return -EFAULT;
	phy->debug_reg_addr = reg_addr;

	return count;
}

static const struct file_operations msm_ssphy_qmp_reg_addr_fops = {
	.open			= msm_ssphy_qmp_reg_addr_open,
	.write			= msm_ssphy_qmp_reg_addr_write,
	.read			= seq_read,
	.llseek			= seq_lseek,
	.release		= single_release,
};

static int msm_ssphy_qmp_reg_value_show(struct seq_file *s, void *unused)
{
	struct msm_ssphy_qmp *phy = s->private;
	u32 reg_value;

	reg_value = readl_relaxed(phy->base + phy->debug_reg_addr - 0x07410000);
...
static int msm_ssphy_qmp_reg_value_open(struct inode *inode, struct file *file)
{
	return single_open(file, msm_ssphy_qmp_reg_value_show, inode->i_private);
}

static ssize_t msm_ssphy_qmp_reg_value_write(struct file *file,
		const char __user *ubuf, size_t count, loff_t *ppos)
{
	struct seq_file		*s = file->private_data;
	struct msm_ssphy_qmp	*phy = s->private;
	...
	ret = kstrtou32_from_user(ubuf, count, 0, &reg_value);
	if (ret)
		return ret;

	if ((phy->debug_reg_addr < 0x07410000) || (phy->debug_reg_addr > 0x07410794))
		return -EFAULT;

	writel_relaxed(reg_value, phy->base + phy->debug_reg_addr - 0x07410000);

	return count;
}

static const struct file_operations msm_ssphy_qmp_reg_value_fops = {
	.open			= msm_ssphy_qmp_reg_value_open,
	.write			= msm_ssphy_qmp_reg_value_write,
	.read			= seq_read,
	.llseek			= seq_lseek,
	.release		= single_release,
};

static const struct debugfs_reg32 msm_ssphy_qmp_regs[] = {
	{"PERIPH_SS_USB3PHY_QSERDES_TX_TX_DRV_LVL", 0x22C},
	{"PERIPH_SS_USB3PHY_QSERDES_TX_TX_EMP_POST1_LVL", 0x218},
	{"PERIPH_SS_USB3PHY_QSERDES_TX_TX_POST2_EMPH ", 0x21C},
	{"PERIPH_SS_USB3PHY_QSERDES_TX_TX_BOOST_LVL_UP_DN", 0x220},
	{"PERIPH_SS_USB3PHY_QSERDES_TX_HP_PD_ENABLES", 0x224},
	{"PERIPH_SS_USB3PHY_QSERDES_RX_RX_EQU_ADAPTOR_CNTRL2", 0x4D8},
};

static int msm_ssphy_qmp_debugfs_init(struct msm_ssphy_qmp *phy)
{
	struct dentry		*debug_root;
	struct dentry		*file;
	int			ret;

	debug_root = debugfs_create_dir(dev_name(phy->phy.dev), NULL);
	if (!debug_root) {
		ret = -ENOMEM;
		goto err0;
	}

	phy->debug_root = debug_root;

	phy->debug_regset = kzalloc(sizeof(*phy->debug_regset), GFP_KERNEL);
	if (!phy->debug_regset) {
		ret = -ENOMEM;
		goto err1;
	}

	phy->debug_regset->regs = msm_ssphy_qmp_regs;
	phy->debug_regset->nregs = ARRAY_SIZE(msm_ssphy_qmp_regs);
	phy->debug_regset->base = phy->base;

	file = debugfs_create_regset32("regdump", S_IRUGO, debug_root, phy->debug_regset);
	if (!file) {
		ret = -ENOMEM;
		goto err1;
	}

	file = debugfs_create_file("reg_addr", S_IRUGO | S_IWUSR, debug_root,
			phy, &msm_ssphy_qmp_reg_addr_fops);
	if (!file) {
		ret = -ENOMEM;
		goto err1;
	}

	file = debugfs_create_file("reg_value", S_IRUGO | S_IWUSR, debug_root,
			phy, &msm_ssphy_qmp_reg_value_fops);
	if (!file) {
		ret = -ENOMEM;
		goto err1;
	}
...
void msm_ssphy_qmp_debugfs_exit(struct msm_ssphy_qmp *phy)
{
	debugfs_remove_recursive(phy->debug_root);
	phy->debug_root = NULL;
}
...
static int msm_ssphy_qmp_probe(struct platform_device *pdev)
{
	...
	msm_ssphy_qmp_debugfs_init(phy);
...
static int msm_ssphy_qmp_remove(struct platform_device *pdev)
{
	...
	msm_ssphy_qmp_debugfs_exit(phy);
