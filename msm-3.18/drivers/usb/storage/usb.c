...
/*Redmine#2748 - #define ITC_ENABLE_WRITE_PROFILING*/

#ifdef ITC_ENABLE_WRITE_PROFILING
#include <linux/time.h>
static int is_write_command = 0;
static struct timespec start_time, end_time;
 static s64 time_taken;
#endif
static int usb_stor_control_thread(void * __us)
{
	...
	for (;;) {
		...
		/* we've got a command, let's do it! */
			...
			#ifdef ITC_ENABLE_WRITE_PROFILING
			if (us->srb->cmnd[0] == 0x2a) {
				int i;
				printk("[ITC] WRITE_10 bytes: ");
				for (i = 0; i < us->srb->cmd_len && i < 16; i++)
					printk(" %02x", us->srb->cmnd[i]);
				printk("\n");
...
SkipForAbort:
		...
		if (is_write_command) {
			 ...
			 time_taken = timespec_to_ns(&end_time) - timespec_to_ns(&start_time);
			 printk("[ITC] WRITE_10 Time Taken = %lld nanosec\n", time_taken);
			 is_write_command = 0;
		}
		#endif
