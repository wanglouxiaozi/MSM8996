...
/*Touch identification(display ident) pin is only connected on Open-Q 820.
  GPIO81 will be  High in case of OSD display and LOW in case of
  Truly(with synaptic touch) display
  */
static int synaptics_dsx_touch_identification(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	const struct synaptics_dsx_board_data *bdata =
		rmi4_data->hw_if->board_data;
	if (!socinfo_get_platform_subtype()) {
		if (gpio_is_valid(bdata->touch_ident_gpio)) {
			retval =  gpio_get_value(bdata->touch_ident_gpio);
		}
	}
	return retval;
}
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	if(synaptics_dsx_touch_identification(rmi4_data)) {
		dev_err(&pdev->dev,
			"%s: Goodix touch detected \n", __func__);
		goto err_set_gpio;
	}
