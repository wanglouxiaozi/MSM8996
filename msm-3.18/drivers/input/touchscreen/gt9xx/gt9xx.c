...
#if 1
static int goodix_ts_pinctrl_init(struct goodix_ts_data *ts_data)
{
	int retval;
...
static void gtp_parse_dt(struct device *dev)
{
	...
	if(!socinfo_get_platform_subtype())
		gtp_ident_gpio = of_get_named_gpio(np, "goodix,touch-ident-gpio", 0);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(!socinfo_get_platform_subtype()) {
		if (gpio_is_valid(gtp_ident_gpio)) {
			/* check touch identification gpio */
			ret = gpio_get_value(gtp_ident_gpio);
			if(ret<=0){
				GTP_ERROR("Synaptic touch detected");
				return -EINVAL;
			}
		}
	}
	...
	if(ret < 0){
		GTP_ERROR("GTP goodix_ts_pinctrl_init Error");
	}
